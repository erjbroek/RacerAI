...
export default class ShopDecoration {
  ...
  public bottom: number = window.innerHeight / 6;

  public lightGrass: GrassLight[] = [];

  public darkGrass: GrassDark[] = [];

  public mushrooms: Mushroom[] = [];

  public red: number = 120;

  public green: number = 220;

  public blue: number = 300;

  public totalTime: number = 0;

  public sunHeight: number = this.bottom + window.innerWidth / 3.7;

  public sunSpeed: number = 0;

  public darkness: number = 0;

  public daySpeed: number = 0.2;
  ...
  public update(frameTime: number) {
    ...
    const cycleDuration = 10000 / this.daySpeed; // Total duration of the cycle (in milliseconds)
    const phaseDuration = cycleDuration / 5; // Duration of each phase (in milliseconds)
    const currentPhase = Math.floor((this.totalTime % cycleDuration) / phaseDuration); // Current phase of the cycle

    const phaseProgress = (this.totalTime % phaseDuration) / phaseDuration; // Progress within the current phase
